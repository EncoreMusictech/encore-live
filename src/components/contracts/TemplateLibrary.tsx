...
p/k/a {{artist_stage_name}}`
  ...
  fields: [
    ...
    { id: 'album_commitment', type: 'number', label: 'Album Commitment', required: false, category: 'work' },
    { id: 'delivery_format', type: 'select', label: 'Delivery Format', required: true, options: ['Digital Masters', 'WAV Files', 'Physical Masters', 'FLAC Files'], category: 'work' },
    
    // Financial Terms - Required
    { id: 'distribution_fee_percent', type: 'number', label: 'Distribution Fee (%)', required: true, category: 'financial' },
    { id: 'fixed_fee_amount', type: 'number', label: 'Fixed Fee Amount ($)', required: false, category: 'financial' },
    { id: 'advance_amount', type: 'number', label: 'Marketing Budget/Advance ($)', required: false, category: 'financial' },
    { id: 'royalty_rate_percent', type: 'number', label: 'Marketing Threshold (%)', required: false, category: 'financial' },
    ...
    // Terms & Conditions - Required
    { id: 'territory', type: 'select', label: 'Territory', required: true, options: ['Worldwide', 'Digital Worldwide', 'North America', 'United States', 'Europe'], category: 'terms' },
    { id: 'term_years', type: 'number', label: 'Term Years', required: true, category: 'terms' },
  ...
  template_data: {
    content: "Distribution Agreement\n\nThis Distribution Agreement (\"Agreement\") is made and entered into as of {{effective_date}}, by and between:\n\n{{company_name}}, located at {{company_address}} (\"Company\"),\nand\n\n{{distributor_name}}, located at {{distributor_address}} (\"Distributor\").\n\n1. GRANT OF RIGHTS\n\nCompany hereby appoints Distributor as its exclusive/non-exclusive distributor to distribute, sell, and otherwise exploit the sound recordings listed in Schedule A (the \"Recordings\") throughout the {{territory}} during the Term.\n\nDistributor shall have the right to:\n\nManufacture, reproduce, and distribute the Recordings in physical and/or digital formats;\n\nPlace the Recordings with retailers, digital service providers (DSPs), and other outlets;\n\nUse Company's approved name, logos, and artwork in connection with promotion and sale of the Recordings.\n\n2. TERM\n\nThis Agreement shall commence on {{effective_date}} and continue for a period of {{term_years}} year(s) (or until completion of {{album_commitment}} album(s), if applicable), unless terminated earlier under Section 10.\n\n3. COMPENSATION\n\nDistributor shall collect all gross receipts (\"Gross Receipts\") from the exploitation of the Recordings. After deducting the agreed-upon distribution fee, Distributor shall pay Company the balance (\"Net Receipts\").\n\nDistribution Fee: {{distribution_fee_percent}}% of Gross Receipts (or flat fee: ${{fixed_fee_amount}})\n\nPayment Schedule: Statements and payments rendered {{accounting_frequency}}, within {{invoice_due_days}} days of period end.\n\nPayments shall be made by {{payment_method}}.\n\n4. DELIVERY REQUIREMENTS\n\nCompany shall deliver to Distributor:\n\nMaster recordings in {{delivery_format}}\n\nArtwork, metadata, and credits in agreed format\n\nNecessary licenses and clearances for exploitation\n\n5. MARKETING & PROMOTION\n\nDistributor may, but is not obligated to, market and promote the Recordings. Any marketing expenses exceeding ${{advance_amount}} or {{royalty_rate_percent}}% of Gross Receipts require Company's prior written approval.\n\n6. OWNERSHIP\n\nAll rights, title, and interest in the Recordings remain the sole property of Company. Distributor acquires no ownership rights other than those expressly granted herein.\n\n7. ACCOUNTING & AUDIT\n\nCompany shall have the right, at its own expense, to audit Distributor's books relating to the Recordings once per year with {{invoice_due_days}} days prior written notice.\n\n8. WARRANTIES & REPRESENTATIONS\n\nEach party warrants that it has the full right, power, and authority to enter into this Agreement.\nCompany warrants that the Recordings do not infringe upon any third-party rights and that all necessary clearances have been obtained.\n\n9. INDEMNIFICATION\n\nEach party agrees to indemnify and hold harmless the other against any losses, damages, or claims arising from a breach of warranties under this Agreement.\n\n10. TERMINATION\n\nEither party may terminate this Agreement upon {{invoice_due_days}} days written notice if the other party materially breaches the Agreement and fails to cure such breach.\n\nUpon termination, Distributor shall cease exploitation of the Recordings and return all materials to Company, subject to fulfilling outstanding commitments and accounting obligations.\n\n11. MISCELLANEOUS\n\nGoverning Law: This Agreement shall be governed by the laws of {{governing_law}}.\n\nNotices: All notices shall be delivered to the addresses listed above.\n\nEntire Agreement: This document contains the full understanding of the parties.\n\nAssignment: Neither party may assign this Agreement without written consent of the other.\n\nSIGNATURES\n\nCompany/Label\nBy: ___________________________\nName: {{company_representative_name}}\nTitle: {{company_representative_title}}\n\nDistributor\nBy: ___________________________\nName: {{company_representative_name}}\nTitle: {{company_representative_title}}\n\nSCHEDULE A – RECORDINGS\n\n(List each title, artist, album/project, catalog number, format, and release date)"
  },
  is_public: true
}, {
  id: '4',
...
const TemplateLibrary: React.FC<TemplateLibraryProps> = ({
  ...
  const generateFallbackTemplate = (contractType: string): string => {
    const templates = {
      ...
      publishing: "Publishing Administration Agreement\n\nThis Publishing Administration Agreement (\"Agreement\") is made and entered into as of {{effective_date}} (the \"Effective Date\"), by and between:\n\n{{writer_legal_name}} (\"Writer\"), IPI/CAE: {{ipi_cae}}, PRO: {{pro_affiliation}}, located at {{company_address}},\n\nand\n\n{{publisher_name}}, at {{publisher_address}} (\"Administrator\"), represented by {{company_representative_name}}, {{company_representative_title}}.\n\n1. TERM\n\nThe term of this Agreement shall commence on the Effective Date and continue for {{term_years}} year(s), unless earlier terminated in accordance with this Agreement (the \"Term\").\n\nAdministrator shall have {{option_periods_count}} option(s) to extend the Term for additional periods upon written notice.\n\n2. GRANT OF RIGHTS\n\nWriter hereby grants to Administrator the sole and exclusive right to administer and exploit the musical compositions listed in Schedule A and any additional compositions created during the Term (the \"Compositions\") throughout the {{territory}}.\n\nAdministrator shall have the right to:\n\nLicense and collect royalties from performances, reproductions, and uses of the Compositions;\n\nEnter into subpublishing agreements;\n\nPrint, publish, and distribute sheet music;\n\nSynchronize Compositions with audiovisual works (subject to Writer approval for sensitive uses such as political, religious, or explicit content);\n\nUse Writer approved name, likeness, and biographical information in connection with exploitation of the Compositions.\n\n3. ADVANCES\n\nAdministrator shall pay Writer an advance of ${{advance_amount}} (the \"Advance\"), recoupable from royalties earned under this Agreement.\n\nAny additional advances, if made, shall also be fully recoupable from Writer royalties.\n\n4. ADMINISTRATION FEE; ROYALTIES\n\nAdministrator shall collect all gross income derived from the Compositions (\"Gross Receipts\"). After deducting the Administration Fee and recoupable amounts, Administrator shall pay Writer the balance of Net Income.\n\nAdministration Fee: {{admin_fee_percent}}%.\n\nPayment Method: {{payment_method}}.\n\n5. PERFORMING RIGHTS\n\nThe performing rights in each Composition shall be licensed and collected by the appropriate performing rights societies (e.g., {{pro_affiliation}}). Administrator shall receive the publisher share, and Writer shall receive the writer share directly.\n\n6. ACCOUNTING\n\nAdministrator shall render royalty statements to Writer on a {{accounting_frequency}} basis within {{invoice_due_days}} days after the end of each accounting period.\n\nPayments shall accompany each statement, subject to recoupment and reserves.\n\nWriter shall have the right, at Writer expense, to audit Administrator books not more than once per year, with at least {{invoice_due_days}} days prior notice.\n\n7. WARRANTIES AND REPRESENTATIONS\n\nWriter represents and warrants that:\n\nWriter owns or controls the rights to the Compositions;\n\nThe Compositions do not infringe upon any other works;\n\nWriter has full authority to enter into this Agreement.\n\nAdministrator represents and warrants that it has the authority to enter into this Agreement.\n\n8. INDEMNITY\n\nEach party agrees to indemnify and hold harmless the other from claims, damages, and expenses arising from any breach of this Agreement.\n\n9. TERMINATION\n\nUpon expiration or termination of the Term:\n\nAdministrator shall continue to collect royalties during the {{term_years}} year(s) Rights Period.\n\nAfter the Rights Period, Administrator may collect royalties for an additional {{option_periods_count}} year(s) Collection Period solely to cover licenses already granted.\n\n10. MISCELLANEOUS\n\nNotices: All notices shall be sent to the addresses set forth above.\n\nEntire Agreement: This Agreement constitutes the entire understanding between the parties.\n\nGoverning Law: This Agreement shall be governed by the laws of {{governing_law}}.\n\nDispute Resolution: Any disputes shall be resolved in the courts of {{jurisdiction}}.\n\nSIGNATURES\n\nWriter:\nName: {{writer_legal_name}}\n\nAdministrator:\nName: {{company_representative_name}}\nTitle: {{company_representative_title}}\n\nSCHEDULE A – COMPOSITIONS\n\n{{song_titles}}",
