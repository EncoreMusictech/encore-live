...
export const CopyrightTable: React.FC<CopyrightTableProps> = ({
  ...
  const getRegistrationStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'not_registered':
      default:
        return <Badge variant="outline">Registered
  ...
  const exportToCSV = () => {
    ...
    // Define CSV headers
    const headers = ['Work ID', 'Work Title', 'ISWC', 'Album Title', 'Masters Ownership', 'Creation Date', 'Copyright Date', 'Registration Status', 'Work Type', 'Language', 'Contains Sample', 'Duration (seconds)', 'Notes', 'Writers', 'Writers Ownership %', 'Controlled Share %', 'ASCAP Work ID', 'ASCAP Status', 'BMI Work ID', 'BMI Status', 'SOCAN Work ID', 'SOCAN Status', 'SESAC Work ID', 'SESAC Status', 'Created At', 'Updated At'];
            ...
            <Table>
              ...
              <TableBody>
                ...
                return <TableRow key={copyright.id} className="hover:bg-muted/30">
                        ...
                        <div className="space-y-1">
                          <div className="flex flex-col space-y-1">
                            <div className="flex items-center gap-1">
                              ...
                              <Badge variant={copyright.ascap_status === 'registered' ? 'default' : copyright.ascap_status === 'pending' ? 'secondary' : 'outline'} className="text-xs">
                                {copyright.ascap_status || 'Not Registered'}
                              </Badge>
                            </div>
                            <div className="flex items-center gap-1">
                              <span className="text-xs font-medium">BMI:</span>
                              <Badge variant={copyright.bmi_status === 'registered' ? 'default' : copyright.bmi_status === 'pending' ? 'secondary' : 'outline'} className="text-xs">
                                {copyright.bmi_status || 'Not Registered'}
                              </Badge>
                            </div>
                            <div className="flex items-center gap-1">
                              <span className="text-xs font-medium">SOCAN:</span>
                              <Badge variant={copyright.socan_status === 'registered' ? 'default' : copyright.socan_status === 'pending' ? 'secondary' : 'outline'} className="text-xs">
                                {copyright.socan_status || 'Not Registered'}
                              </Badge>
                            </div>
                            <div className="flex items-center gap-1">
                              <span className="text-xs font-medium">SESAC:</span>
                              <Badge variant={copyright.sesac_status === 'registered' ? 'default' : copyright.sesac_status === 'pending' ? 'secondary' : 'outline'} className="text-xs">
                      ...
                      <TableCell>
                        {onEdit && <Button variant="outline" size="sm" onClick={() => onEdit(copyright)} className="h-8 w-8 p-0">
