...
export function SongEstimatorTool() {
  ...
  return <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="flex gap-4">
            <div className="flex-1">
              ...
              <Input id="songwriter" value={songwriterName} onChange={e => setSongwriterName(e.target.value)} placeholder="e.g., Max Martin, Diane Warren, Ryan Tedder" onKeyPress={e => e.key === 'Enter' && handleCreateSearch()} />
            </div>
            <div className="flex items-end">
              <Button onClick={handleCreateSearch} disabled={!songwriterName.trim() || loading} className="bg-primary hover:bg-primary/90">
                {loading ? <>
      ...
      {searches.length > 0 && <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {searches.map(search => <div key={search.id} className={`p-4 border rounded-lg cursor-pointer hover:bg-muted/50 transition-colors ${selectedSearchId === search.id ? 'bg-muted border-primary' : ''}`} onClick={() => handleSelectSearch(search)}>
      ...
      {/* Detailed View */}
      {currentSearch && <Tabs defaultValue="overview" className="space-y-4">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Button onClick={() => runBulkBMIVerification(currentSearch.id)} disabled={bmiVerificationLoading || !songMetadata.length} variant="outline" size="sm">
                      {bmiVerificationLoading ? <>
                    ...
                    <div className="p-3 border rounded-lg bg-card">
                      <div className="flex items-center justify-between">
                        ...
                        <Badge variant="outline">
                          {songMetadata.length > 0 ? Math.round(songMetadata.filter(song => song.verification_status === 'bmi_verified').length / songMetadata.length * 100) : 0}%
                        </Badge>
                      </div>
                    </div>
                  </div>
                ...
                {currentSearch.ai_research_summary && <div className="space-y-4">
                    <h4 className="font-semibold">AI Research Summary</h4>
                    {currentSearch.ai_research_summary.career_overview && <div className="p-4 border rounded-lg bg-muted/50">
                        <h5 className="font-medium mb-2">Career Overview</h5>
                        <p className="text-sm text-muted-foreground">
                          {currentSearch.ai_research_summary.career_overview.summary}
                        </p>
                      </div>}
                    {currentSearch.ai_research_summary.pipeline_summary && <div className="p-4 border rounded-lg bg-card">
          ...
          <TabsContent value="catalog">
            <SongMetadataView searchId={currentSearch.id} songMetadata={songMetadata} />
          </TabsContent>

          <TabsContent value="pipeline">
            <PipelineEstimateView searchId={currentSearch.id} songMetadata={songMetadata} />
          </TabsContent>
