...
interface BulkContractImportJob {
  id: string;
  file_name: string;
...
interface BulkContractImportHistoryProps {
  companyId: string;
}

export function BulkContractImportHistory({ companyId }: BulkContractImportHistoryProps) {
  const [jobs, setJobs] = useState<BulkContractImportJob[]>([]);
  ...
  const fetchJobs = async () => {
    try {
      ...
      setJobs((data as unknown as BulkContractImportJob[]) || []);
    } catch (error) {
      console.error('Error fetching contract import jobs:', error);
  ...
  const handleDeleteJob = async (job: BulkContractImportJob) => {
    try {
      setDeleting(job.id);
      await supabase.from('bulk_contract_import_jobs').delete().eq('id', job.id);
      setJobs(prev => prev.filter(j => j.id !== job.id));
      toast({ title: 'Job Deleted', description: `Removed import record "${job.file_name}".` });
    } catch (error) {
      console.error('Error deleting job:', error);
      toast({ title: 'Error', description: 'Failed to delete import job', variant: 'destructive' });
    } finally {
      setDeleting(null);
    }
  };
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-start justify-between">
        <div>
          <CardTitle>Contract Import History</CardTitle>
          <CardDescription>
            Past bulk contract imports for this sub-account ({jobs.length} total)
          </CardDescription>
        </div>
      ...
      <CardContent>
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground">No bulk contract imports yet</p>
          </div>
        ) : (
          <div className="border rounded-lg">
            <Table>
              ...
              <TableBody>
                {jobs.map((job) => {
                  const hasErrors = job.failed_contracts > 0 && job.error_log && job.error_log.length > 0;
                    ...
                    <Collapsible key={job.id} open={isExpanded} onOpenChange={() => hasErrors && toggleExpanded(job.id)} asChild>
                      <>
                        <TableRow className={hasErrors ? 'cursor-pointer' : ''}>
                          ...
                          <TableCell className="text-right text-primary font-medium">{job.successful_contracts}</TableCell>
                          <TableCell className="text-right text-amber-600 font-medium">{job.updated_contracts}</TableCell>
                          <TableCell className="text-right text-muted-foreground">{job.skipped_contracts}</TableCell>
                          <TableCell className="text-right">
                            {job.failed_contracts > 0 ? (
                              <span className="text-destructive font-medium cursor-pointer hover:underline" onClick={() => hasErrors && toggleExpanded(job.id)}>
                                {job.failed_contracts}
                              </span>
                            ) : (
                              <span className="font-medium">0</span>
                            )}
                          </TableCell>
                          <TableCell className="text-right">{job.total_contracts}</TableCell>
                          <TableCell>
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  <AlertDialogTitle>Delete this import record?</AlertDialogTitle>
                                  <AlertDialogDescription>
                                    This will remove the import history record for "{job.file_name}". The contracts themselves will not be deleted.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                ...
                                <div className="bg-destructive/5 border-t border-b border-destructive/10 px-6 py-4">
                                  <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <span className="text-sm font-semibold text-destructive">
                                      {job.error_log.length} Failed Contract{job.error_log.length !== 1 ? 's' : ''}
                                    </span>
                                  </div>
                                    ...
                                    <Table>
                                      <TableHeader>
                                        <TableRow className="bg-destructive/5 hover:bg-destructive/5">
                                          ...
                                          <TableHead className="text-xs">Contract Title</TableHead>
