...
DROP POLICY IF EXISTS "Operations team can manage performance metrics" ON public.performance_monitoring_metrics;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_monitoring_events_created_at') THEN
        CREATE INDEX idx_monitoring_events_created_at ON public.realtime_monitoring_events(created_at DESC);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_monitoring_events_severity') THEN
        CREATE INDEX idx_monitoring_events_severity ON public.realtime_monitoring_events(severity);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_monitoring_events_status') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_realtime_monitoring_events_updated_at') THEN
