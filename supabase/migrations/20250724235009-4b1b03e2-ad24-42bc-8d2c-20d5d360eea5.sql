...
-- Add client-portal access for admin user using correct access_source
DO $$
DECLARE
    admin_user_id UUID;
BEGIN
    ...
    -- Insert client-portal module access if user exists (using demo_access as allowed source)
    IF admin_user_id IS NOT NULL THEN
        INSERT INTO public.user_module_access (user_id, module_id, access_source, granted_at)
        VALUES (admin_user_id, 'client-portal', 'demo_access', now())
