...
FOR ALL USING (is_operations_team_member(auth.uid()) OR (is_shared = true AND auth.uid() IS NOT NULL));
...
CREATE INDEX idx_bi_reports_shared ON public.business_intelligence_reports(is_shared);
