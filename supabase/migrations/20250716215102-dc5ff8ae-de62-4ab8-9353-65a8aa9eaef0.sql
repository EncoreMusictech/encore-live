...
BEGIN
    LOOP
        statement_id := 'STMT-' || to_char(CURRENT_DATE, 'YYYY') || '-' || LPAD(counter::text, 4, '0');
        
        -- Check if this ID already exists
        IF NOT EXISTS (SELECT 1 FROM public.royalties_import_staging WHERE statement_id = statement_id) THEN
            RETURN statement_id;
        END IF;
