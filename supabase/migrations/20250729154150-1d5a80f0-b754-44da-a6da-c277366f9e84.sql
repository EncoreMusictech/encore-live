...
-- Fix ISWCs using a subquery approach
WITH numbered_copyrights AS (
  SELECT id, ROW_NUMBER() OVER (ORDER BY created_at) as row_num
  FROM copyrights
  WHERE iswc IS NULL OR iswc = ''
)
UPDATE copyrights 
SET iswc = 'T' || LPAD(nc.row_num::text, 9, '0') || '1'
FROM numbered_copyrights nc
WHERE copyrights.id = nc.id;