-- Enable RLS on catalog_revenue_sources (if table exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 
  ...
  ELSE
    RAISE NOTICE 'Table public.catalog_revenue_sources not found. Skipping.';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    IF NOT EXISTS (
      ...
      WHERE schemaname = 'public' AND tablename = 'catalog_revenue_sources' 
        AND policyname = 'Users can insert their own revenue sources'
    ) THEN
...
BEGIN
  IF EXISTS (
    ...
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies 
      WHERE schemaname = 'public' AND tablename = 'catalog_revenue_sources' 
        AND policyname = 'Users can update their own revenue sources'
    ) THEN
...
BEGIN
  IF EXISTS (
    ...
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies 
      WHERE schemaname = 'public' AND tablename = 'catalog_revenue_sources' 
        AND policyname = 'Users can select their own revenue sources'
    ) THEN
...
BEGIN
  IF EXISTS (
    ...
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies 
      WHERE schemaname = 'public' AND tablename = 'catalog_revenue_sources' 
        AND policyname = 'Users can delete their own revenue sources'
    ) THEN
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='catalog_revenue_sources'
  ) THEN
    IF NOT EXISTS (
      SELECT 1 FROM pg_trigger WHERE tgname = 'update_catalog_revenue_sources_updated_at'
    ) THEN
