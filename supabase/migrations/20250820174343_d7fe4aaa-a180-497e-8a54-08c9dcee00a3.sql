-- Drop and recreate get_client_quarterly_balances to include contract data
...
BEGIN
  ...
  WITH qb AS (
    SELECT
      ...
      -- Get the contract details from the first non-null contract in the group
      MAX(c.agreement_id) AS agreement_id,
      MAX(c.title) AS contract_title
    FROM public.quarterly_balance_reports q
    LEFT JOIN public.payees p ON p.id = q.payee_id
    LEFT JOIN public.contracts c ON c.id = q.agreement_id
  ...
  SELECT 
    ...
    qb.agreement_id AS agreement_id,
    qb.contract_title AS contract_title
  FROM qb
