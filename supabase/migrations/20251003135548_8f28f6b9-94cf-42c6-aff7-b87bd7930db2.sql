-- Retry: fix parentheses in RPC update
...
DECLARE
  ...
  royalty_assoc_payee_ids uuid[] := ARRAY[]::uuid[];
  client_contact_ids uuid[] := ARRAY[]::uuid[];
BEGIN
  ...
  -- 2) Royalty allocation associations -> derive payee_ids via payouts linkage
  SELECT COALESCE(ARRAY_AGG(DISTINCT p.payee_id), ARRAY[]::uuid[])
  INTO royalty_assoc_payee_ids
  FROM public.payouts p
  JOIN public.payout_royalties pr ON pr.payout_id = p.id
  JOIN public.royalty_allocations ra ON ra.id = pr.royalty_id
  JOIN public.client_data_associations cda 
    ON cda.data_id = ra.id 
   AND cda.data_type = 'royalty_allocation'
   AND cda.client_user_id = current_user_id
  WHERE p.user_id = subscriber_id;

  -- Merge arrays
  client_payee_ids := (
    SELECT ARRAY(
      SELECT DISTINCT unnest(COALESCE(client_payee_ids, ARRAY[]::uuid[]))
      UNION
      SELECT DISTINCT unnest(COALESCE(royalty_assoc_payee_ids, ARRAY[]::uuid[]))
    )
  );

  -- 3) Contacts tied to this client access and match payees by name for the subscriber
  ...
  IF array_length(client_contact_ids,1) > 0 THEN
    client_payee_ids := (
      SELECT ARRAY(
  ...
  -- 4) Fallback via matching client email -> contact email
