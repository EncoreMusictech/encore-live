...
DROP POLICY IF EXISTS "Operations team can manage monitoring events" ON public.realtime_monitoring_events;
DROP POLICY IF EXISTS "Operations team can manage automation workflows" ON public.automation_workflows;
DROP POLICY IF EXISTS "Operations team can manage BI reports" ON public.business_intelligence_reports;
DROP POLICY IF EXISTS "Operations team can manage system alerts" ON public.system_alerts;
DROP POLICY IF EXISTS "Operations team can manage integrations" ON public.integration_management;
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_monitoring_events_created_at ON public.realtime_monitoring_events(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_monitoring_events_severity ON public.realtime_monitoring_events(severity);
CREATE INDEX IF NOT EXISTS idx_monitoring_events_status ON public.realtime_monitoring_events(status);
...
CREATE INDEX IF NOT EXISTS idx_integrations_status ON public.integration_management(connection_status);
CREATE INDEX IF NOT EXISTS idx_integrations_active ON public.integration_management(is_active);
