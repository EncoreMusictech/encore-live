-- Fix the calculate_payout_fields function to properly separate commission fees from recoupable expenses
...
BEGIN
    ...
    -- Calculate commissions based on agreement percentage
    total_commissions := COALESCE(total_royalties_amount * (agreement_commission / 100.0), 0);
    
    -- Calculate commission fees from expenses (separate from percentage-based commissions)
    SELECT COALESCE(SUM(amount), 0)
    INTO commission_fees
    ...
    AND (is_commission_fee = true OR (expense_flags->>'commission_fee')::boolean = true);
    
    -- Add commission fees to total commissions
    total_commissions := total_commissions + commission_fees;
    
    -- Calculate recoupable expenses (excluding commission fees)
    SELECT COALESCE(SUM(amount), 0)
    INTO total_exp
    FROM public.payout_expenses
    WHERE (payout_id = NEW.id OR (payee_id = client_payee_id AND payout_id IS NULL))
    ...
    AND NOT (is_commission_fee = true OR (expense_flags->>'commission_fee')::boolean = true);
    ...
    -- Calculate Royalties to Date as cumulative sum of GROSS ROYALTIES (ITD - Inception To Date)
    ...
    royalties_cumulative := COALESCE(royalties_cumulative + (total_royalties_amount - total_commissions), (total_royalties_amount - total_commissions));
    
    -- Set calculated fields
    NEW.total_royalties := COALESCE(total_royalties_amount, 0);
    NEW.commissions_amount := COALESCE(total_commissions, 0);
    NEW.gross_royalties := COALESCE(total_royalties_amount - total_commissions, NEW.gross_royalties, 0);
